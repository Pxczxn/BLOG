...
const CommunityPostComment = () => {
  ...
  const fetchComments = async (page = 1, size = 10) => {
    setLoading(true);
    try {
      const res = await request.get('/api/admin/community/comments', {
  ...
  const handleBatchAction = async (action) => {
    ...
    const results = await Promise.allSettled(selectedRowKeys.map((id) => request.put(`/api/admin/community/comments/${id}/${action}`)));
  ...
  return (
    <div className="admin-page admin-page--table">
      <div className="admin-page__header">
        <div>
          <h1 className="admin-page__title">帖子评论管理</h1>
