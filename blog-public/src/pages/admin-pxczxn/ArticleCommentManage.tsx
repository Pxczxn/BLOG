...
export default function ArticleCommentManage() {
  ...
  const handleDelete = async (id: number) => {
    if (!window.confirm('确定要删除这条文章评论吗？')) return;
          ...
          <table className="w-full border-collapse text-left">
            <thead>
              <tr className="border-b border-white/10 bg-black/20 text-xs uppercase tracking-wider text-slate-400">
                ...
                <th className="px-6 py-4 font-medium">文章</th>
            ...
            <tbody className="divide-y divide-white/5 text-sm">
              {loading ? (
                ...
                comments.map((comment) => (
                  <tr key={comment.id} className="hover:bg-white/5">
                    ...
                    <td className="px-6 py-4 text-slate-400">
                      <div className="font-medium text-white">{comment.articleTitle || `文章 #${comment.articleId}`}</div>
                      <div className="text-xs text-slate-500">ID: {comment.articleId}</div>
                    </td>
                ...
                <tr><td colSpan={6} className="px-6 py-16 text-center text-slate-500">暂无文章评论</td></tr>
              )}
            </tbody>
          </table>
        </div>
