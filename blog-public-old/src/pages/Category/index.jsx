...
import { ArrowRight, NotebookPen, Eye, Folder, ArrowLeft } from 'lucide-react';
...
const CategoryPage = () => {
    ...
    useEffect(() => {
        const fetchCategoryInfo = async () => {
            try {
                ...
                const catItems = Array.isArray(resp.data) ? resp.data : (resp.data?.items || []);
                const currentCat = catItems.find((c) => String(c.id) === String(slug) || c.slug === slug);
                setCategoryName(currentCat?.name || `分类 ${slug}`);
            } catch (error) {
        ...
        const fetchArticles = async (currentPage) => {
            setLoading(true);
            try {
                const resp = await request.get('/api/public/articles', { params: { page: currentPage, size: 10, categoryId: slug } });
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-8 duration-1000 font-sans">
            <Helmet>
                <title>{categoryName} - BLOG | 分类</title>
                <meta name="description" content={`浏览分类 ${categoryName} 下的所有文章记录。`} />
            </Helmet>
            ...
            <div className="mb-12 md:mb-16 text-center border-b border-white/5 pb-12">
                ...
                <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-slate-100 mb-6 font-serif">{categoryName}</h1>
                <p className="text-lg text-slate-400 leading-relaxed font-light">
                    共收录 <span className="font-semibold text-indigo-400 mx-1">{data.total || 0}</span> 篇文章
                </p>
            </div>
