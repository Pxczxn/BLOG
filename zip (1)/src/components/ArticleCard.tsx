...
export default function ArticleCard({ article, delay = 0 }: { article: Article; delay?: number }) {
  return (
    ...
    >
      <Link 
        to={`/post/${article.id}`} 
        className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col sm:flex-row gap-5 backdrop-blur-md hover:bg-white/10 transition-all cursor-pointer group relative"
      >
        {article.isPinned && (
